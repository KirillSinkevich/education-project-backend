...
class UserController {
  async registration(req, res, next){
    try {
      ...
      next(ApiError.BedRequest('Validation error', e.errors))
    }
  }
  ...
  async login(req, res, next){
    try {
      ...
      // res.cookie('refreshToken', userData.refreshToken, {maxAge: 5000, httpOnly: true});
      return res.json(userData);
    } catch (e) {
      next(e)
    }
  ...
  async refresh(req, res, next){
    try {
      ...
      res.cookie('refreshToken', userData.refreshToken, {maxAge: 15 * 24 * 3600 * 1000, httpOnly: true});
      // res.cookie('refreshToken', userData.refreshToken, {maxAge: 5000, httpOnly: true});
      return res.json(userData);
    } catch (e) {
      next(e)
